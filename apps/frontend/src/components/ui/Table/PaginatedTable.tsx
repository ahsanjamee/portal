...
type IPaginatedTableProps = {
  ...
  setColumnFilterFns?: React.Dispatch<
    React.SetStateAction<MRT_ColumnFilterFnsState>
  >;
  globalFilter?: string;
  ...
  renderTopToolbarCustomActions?:
    | ((props: { table: MRT_TableInstance<MRT_RowData> }) => React.ReactNode)
    | undefined;
  onRowClick?: (row: any) => void;
};

const PaginatedTable: React.FC<IPaginatedTableProps> = ({
  data,
  error,
  columns,
  ...props
  ...
  const table = useMantineReactTable({
    ...
    mantineTableBodyRowProps: ({ row }) => ({
      ...props.mantineTableBodyRowProps,
      onClick: props.onRowClick
        ? () => props.onRowClick?.(row.original)
        : undefined,
      style: props.onRowClick ? { cursor: "pointer" } : {},
    }),
    ...
    icons: {
      IconArrowsSort: () => (
        <CaretUpDown size={16} color="#667085" weight="bold" />
      ),
    },
